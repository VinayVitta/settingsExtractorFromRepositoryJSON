    ...
    for database in databases:
        if database['name'] == source_ep_name and database['type_id'] == 'ORACLE_COMPONENT_TYPE':
            ...
            row_data = {
                ...
                'orc_source_service': orc_source_service,
                'useLogminerReader': db_settings.get('useLogminerReader'),
                'asm_server': asm_server[0] if asm_server else None,
                'asm_user': db_settings.get('asm_user'),
                'useBfile': db_settings.get('useBfile', 'default'),
                'addSupplementalLogging': db_settings.get('addSupplementalLogging', 'default'),
                'accessAlternateDirectly': db_settings.get('accessAlternateDirectly', 'default'),
                'readAheadBlocks': db_settings.get('readAheadBlocks', 'default'),
                'archivedLogDestId': db_settings.get('archivedLogDestId', 'default'),
                'securityDbEncryption': 'Enabled' if db_settings.get('securityDbEncryption') else 'Disabled',
                'additionalArchivedLogDestId': db_settings.get('additionalArchivedLogDestId'),
                'useZeroDestid': db_settings.get('useZeroDestid'),
                'asmUsePLSQLArray': db_settings.get('asmUsePLSQLArray'),
                'skipValidationLongNames': db_settings.get('skipValidationLongNames'),
                'parallelASMReadThreads': db_settings.get('parallelASMReadThreads'),
